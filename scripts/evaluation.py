...
in_file = 'answer.jsonl'
out_file = 'evaluation.jsonl'
...
processed_ids = set()
try:
    with open(out_file, 'r', encoding='utf-8') as outfile:
        for line in outfile:
            record = json.loads(line.strip())
            if 'id' in record:
                processed_ids.add(record['id'])
except FileNotFoundError:
    print("Answer file not found, starting fresh.")


dic_gt = {}
json_data = []
for ori_file in ori_files:
    with open(ori_file, 'r', encoding='utf-8') as ori_f:
        data = json.load(ori_f)
        for item in data:
            q_item = item['query']
            q_id = q_item['id']
            q_ans = q_item['answer']
...
with open(in_file, 'r', encoding='utf-8') as in_f, open(out_file, 'a', encoding='utf-8') as out_f:
    for line in in_f:
        ...
        reference_ori = dic_gt[record_id]

        reference_list = reference_ori.split("|")
        scoring_function = initialize_encyclopedic_vqa_evaluation_function()
        scores = []
        
        record_ans = record['answer'][:2000] if len(record['answer']) > 2000 else record['answer']

        for reference in reference_list:
            example = {
                'question': question,
                'reference': reference,
                'candidate': record_ans,
        ...
        # Optional: flush the buffer to ensure writing
        # Note: This might impact performance if processing many small items
        out_f.flush()
