    ...
    with open(query_path, 'r', encoding='utf-8') as infile:
        for line_number, line in enumerate(tqdm(infile, total=total_lines, desc="Processing", unit="line"), start=1):
            with open(answer_path, 'a', encoding='utf-8') as outfile:
                ...
                #     f"Here are only one reference material, it contains an image, wiki title, and description:\n\n"
                #     f"\n{ref_content_A}\n"
                ...
                question = (
                    f"Based on the retrieved document, answer the question <image>\n {question_ori} within 5 words\n\n"
                    f"context A: {ref_content_A}\n"
                    f"context B: {ref_content_B}\n"
                    f"context C: {ref_content_C}\n"
                    f"context D: {ref_content_D}\n"
                    f"context E: {ref_content_E}\n"
                )
